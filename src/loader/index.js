...
async function connectMongoDB() {
  ...
  mongoose.connection.on('connecting', () => {
    logger.info('Mongoose가 MongoDB 서버에 연결중입니다!');
  });
  mongoose.connection.on('connected', () => {
    logger.info('Mongoose가 MongoDB에 정상적으로 연결되었습니다.');
  });
  mongoose.connection.on('disconnecting', () => {
    logger.info('Mongoose가 MongoDB와의 연결을 끊고 있습니다!');
  });
  mongoose.connection.on('disconnected', () => {
    logger.info('Mongoose가 MongoDB와의 연결을 정상적으로 끊었습니다.');
  });
  mongoose.connection.on('error', (error) => {
    logger.info(`Mongoose에서 에러가 발생하였습니다: ${error}`);
  });
